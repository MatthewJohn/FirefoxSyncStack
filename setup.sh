...
# Generates AUTO_PUSH_CRYPTO_KEY
export AUTO_PUSH_$(pushd /autopush >/dev/null ; echo 'from autokey import main; main()' | python; popd >/dev/null)

DYNAMODB_INTERNAL_HOST=localhost
DYNAMODB_INTERNAL_PORT=8051
DYNAMODB_INTERNAL_URL=http://\${DYNAMODB_INTERNAL_HOST}:\${DYNAMODB_INTERNAL_PORT}
...
export AUTO_PUSH_INTERNAL_HOST=localhost
export AUTO_PUSH_INTERNAL_PORT=8978
export AUTO_PUSH_INTERNAL_URL=http://\${AUTO_PUSH_INTERNAL_HOST}:\${AUTO_PUSH_INTERNAL_PORT}
export AUTO_PUSH_EXTERNAL_DOMAIN=push.\${BASE_DOMAIN}
export AUTO_PUSH_EXTERNAL_URL=https://\${AUTO_PUSH_EXTERNAL_DOMAIN}

export AUTO_PUSH_ENDPOINT_INTERNAL_HOST=localhost
export AUTO_PUSH_ENDPOINT_INTERNAL_PORT=8979
export AUTO_PUSH_ENDPOINT_INTERNAL_URL=http://\${AUTO_PUSH_ENDPOINT_INTERNAL_HOST}:\${AUTO_PUSH_ENDPOINT_INTERNAL_PORT}
export AUTO_PUSH_ENDPOINT_EXTERNAL_DOMAIN=updates.push.\${BASE_DOMAIN}
export AUTO_PUSH_ENDPOINT_EXTERNAL_URL=https://\${AUTO_PUSH_ENDPOINT_EXTERNAL_DOMAIN}
...
cat > /autopush/configs/config.ini <<EOF
...
crypto_key = [${AUTO_PUSH_CRYPTO_KEY}]
...
hostname = ${AUTO_PUSH_EXTERNAL_DOMAIN}
port = ${AUTO_PUSH_INTERNAL_PORT}
;
...
endpoint_hostname = ${AUTO_PUSH_ENDPOINT_EXTERNAL_DOMAIN}
endpoint_port = ${AUTO_PUSH_ENDPOINT_INTERNAL_PORT}
...
server {
  listen 443 ssl;
  server_name ${AUTO_PUSH_EXTERNAL_DOMAIN};
  ...
  location / {
    ...
    proxy_read_timeout 120;
    proxy_connect_timeout 10;
    proxy_pass ${AUTO_PUSH_INTERNAL_URL}/;
   }
}
server {
  listen 443 ssl;
  server_name ${AUTO_PUSH_ENDPOINT_EXTERNAL_DOMAIN};
  ...
  location / {
    ...
    proxy_read_timeout 120;
    proxy_connect_timeout 10;
    proxy_pass ${AUTO_PUSH_ENDPOINT_INTERNAL_URL}/;
   }
}
server {
  listen 443 ssl;
...
sleep 10
/usr/bin/java -Djava.library.path=. -jar DynamoDBLocal.jar -dbPath /var/dynamodb_local -port ${DYNAMODB_INTERNAL_PORT} &
sleep 20
pushd /autopush; . ./bin/activate;
    AWS_LOCAL_DYNAMODB=${DYNAMODB_INTERNAL_URL} autopush --config-connection /autopush/configs/config.ini &
    AWS_LOCAL_DYNAMODB=${DYNAMODB_INTERNAL_URL} autoendpoint --config-connection /autopush/configs/config.ini &
    deactivate; popd
sleep 10
